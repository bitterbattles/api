...
import (
	...
	. "github.com/bitterbattles/api/lambdas/getbattles"
)
...
func TestHandlerFullPage(t *testing.T) {
	...
	addBattles(index, repository, sort, 3)
	...
	testHandler(t, index, repository, sort, 1, 2, expectedResponse)
}
...
func TestHandlerLastPage(t *testing.T) {
	...
	testHandler(t, index, repository, sort, 2, 2, expectedResponse)
}
...
func TestHandlerBeyondLastPage(t *testing.T) {
	...
	addBattles(index, repository, sort, 3)
	expectedResponse := `[]`
	testHandler(t, index, repository, sort, 3, 2, expectedResponse)
}

func TestHandlerNoPagination(t *testing.T) {
	index := mocks.NewIndex()
	repository := mocks.NewRepository()
	addBattles(index, repository, sort, 50)
	...
	testHandler(t, index, repository, sort, 0, 0, expectedResponse)
}

func TestHandlerTooLargePage(t *testing.T) {
	index := mocks.NewIndex()
	repository := mocks.NewRepository()
	addBattles(index, repository, sort, 101)
	...
	testHandler(t, index, repository, sort, 1, 101, expectedResponse)
}
...
func TestHandlerNoSort(t *testing.T) {
	...
	testHandler(t, index, repository, "", 1, 1, expectedResponse)
}
...
func TestHandlerBadSort(t *testing.T) {
	...
	addBattles(index, repository, sort, 3)
	expectedResponse := `[{"id":"id0","title":"title0","description":"description0","votesFor":0,"votesAgainst":0,"createdOn":0}]`
	testHandler(t, index, repository, "bad", 1, 1, expectedResponse)
}

func TestHandlerBadIndexEntry(t *testing.T) {
	index := mocks.NewIndex()
	index.Set(sort, "badID", 0)
	repository := mocks.NewRepository()
	addBattles(index, repository, sort, 1)
	expectedResponse := `[{"id":"id0","title":"title0","description":"description0","votesFor":0,"votesAgainst":0,"createdOn":0}]`
	testHandler(t, index, repository, sort, 1, 2, expectedResponse)
}

func addBattles(index *mocks.Index, repository *mocks.Repository, indexName string, count int) {
	for i := 0; i < count; i++ {
		...
		index.Set(indexName, battle.ID, float64(i))
	}
}
